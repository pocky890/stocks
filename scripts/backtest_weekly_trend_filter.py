"""研究用一次性腳本：比較atr_breakout/breakout(唐奇安通道突破型策略)加上「週線趨勢確認」
濾網前後的表現——使用者建議日線突破進場太容易遇到假突破，額外要求週線MA20斜率向上(或
收盤站上週MA20)才進場。全觀察清單10年+2026 YTD/7月回測，不動STRATEGY_REGISTRY的
預設params。"""
import sys
...
CANDIDATES = [ATRBreakoutStrategy(), BreakoutStrategy()]
CONFIGS = [
    ("原本(無週線濾網)", {}),
    ("+週線MA20斜率向上", {"require_weekly_trend": True, "weekly_trend_mode": "slope"}),
    ("+收盤站上週MA20", {"require_weekly_trend": True, "weekly_trend_mode": "above_ma"}),
]
