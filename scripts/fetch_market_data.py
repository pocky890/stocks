"""補三大法人買賣超、估值(PE/殖利率/PB)、除權息預告表——全部是免費公開資料，
跟Shioaji帳戶/金鑰狀態無關，現在就能跑。

三大法人/估值的歷史回補全部改用FinMind(一支股票一次查詢就拿到整段歷史範圍)，
...
def main():
    ...
    print(f"用FinMind補三大法人/估值歷史 {start_date}~{end_date}...")
    ...
    for symbol in symbols:
        ...
        print(f"  {symbol}: 籌碼{len(flows)}筆 / 估值{len(valuations)}筆")
