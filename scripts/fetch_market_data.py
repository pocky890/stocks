"""補三大法人買賣超、估值(PE/殖利率/PB)、月營收、除權息預告表——全部是免費公開資料，
跟Shioaji帳戶/金鑰狀態無關，現在就能跑。

三大法人/估值/月營收的歷史回補全部改用FinMind(一支股票一次查詢就拿到整段歷史範圍)，
...
的清單，不是歷史資料，跟這裡其他backfill的性質不同)。融資融券資料2026-08-16拿掉
(dashboard圖表用不到，改成成交量子圖)，不再抓取。
"""
import sys
...
def main():
    ...
    print(f"用FinMind補三大法人/估值/月營收歷史 {start_date}~{end_date}...")
    ...
    for symbol in symbols:
        ...
        print(f"  {symbol}: 籌碼{len(flows)}筆 / 估值{len(valuations)}筆 / 月營收{len(revenue)}筆")
