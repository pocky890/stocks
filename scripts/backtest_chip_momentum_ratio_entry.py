"""研究用一次性腳本：比較chip_momentum(外資買超動能)現行「連續剛好5天買超」進場條件
vs 使用者提出的「近5日外資淨買超加總為正 + 淨買超加總/近5日總成交量加總>X%」新條件
(entry_mode="ratio")，測試X%=6/8/10/12四種門檻，全觀察清單10年+2026 YTD/7月回測比較，
不動STRATEGY_REGISTRY的預設params。"""
import sys
...
def build_configs(base_params):
    return [
        ("現行config.json(連續剛好5天)", base_params),
        ("ratio 6%", {**base_params, "entry_mode": "ratio", "ratio_window_days": 5, "ratio_threshold": 0.06}),
        ("ratio 8%", {**base_params, "entry_mode": "ratio", "ratio_window_days": 5, "ratio_threshold": 0.08}),
        ("ratio 10%", {**base_params, "entry_mode": "ratio", "ratio_window_days": 5, "ratio_threshold": 0.10}),
        ("ratio 12%", {**base_params, "entry_mode": "ratio", "ratio_window_days": 5, "ratio_threshold": 0.12}),
    ]
...
def main():
    config = load_config()
    strategy = ChipMomentumStrategy()
    base_params = config.strategy_params["chip_momentum"]
    ...
    for scope_label, start, end in [("全觀察清單10年", None, None), ("2026 YTD", YTD_START, None), ("2026-07單月", JULY_START, JULY_END)]:
        ...
        for label, extra in configs:
            ...
            for code, name in symbols:
                ...
                if bars.empty or "foreign_net" not in bars.columns:
                    continue
