"""研究用一次性腳本：比較trend_following(趨勢追蹤)現行的edge-triggered進場(條件剛從False
轉True那天才觸發) vs level-triggered(條件當天成立就觸發，不要求邊緣)，全觀察清單10年+
2026 YTD/7月回測比較，不動STRATEGY_REGISTRY的預設params。動機：trend_following跟
long_swing/trust_momentum一樣是「多頭排列+動能濾網」形狀，理論上有同一種「停損出場後
條件還沒轉一輪False→True就卡住進不了場」的風險，值得驗證改成level-triggered後表現有沒有
變化。"""
import sys
