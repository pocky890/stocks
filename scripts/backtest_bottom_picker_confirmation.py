"""研究用一次性腳本：bullish_divergence/chip_reversal_fast 2026年7月系統性重挫期間
表現很差，追查後發現訊號當天就進場、沒有等隔天確認，進場後平均還要再跌10~28個交易日
才真正落底(見對話紀錄的extra_drop_after_entry分析)。capitulation_reversal同一段期間
完全沒有誤觸發，差別就在那支策略本來就有的「隔天不再破前低+收盤收高才進場」確認機制——
這裡把同一套確認邏輯借來測，比較加confirm_next_day前後的全觀察清單10年表現，以及
2026年YTD/7月單獨表現，不動STRATEGY_REGISTRY的預設params。"""
import sys
