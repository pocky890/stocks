"""研究用一次性腳本：比較breakout(突破)現行的edge-triggered進場(條件剛從False轉True那天
才觸發) vs level-triggered(條件當天成立就觸發，不要求邊緣)，全觀察清單10年+2026 YTD/7月
回測比較，不動STRATEGY_REGISTRY的預設params。動機同
backtest_trend_following_entry_trigger.py：強勢單邊行情「創新高+爆量」可能連續多天成立，
edge-triggered有機會複製long_swing/trust_momentum的停損出場卡住問題。"""
import sys
...
BASE_PARAMS = {
    ...
    "low_lookback_days": 10,
    "volume_avg_period": 20,
    "volume_multiplier": 1.5,
    "atr_period": 14,
    "atr_multiplier": 2,
    "require_weekly_trend": True,
    "weekly_trend_mode": "slope",
}
CONFIGS = [
    ("原本(edge-triggered)", {**BASE_PARAMS, "entry_trigger": "edge"}),
    ("level-triggered", {**BASE_PARAMS, "entry_trigger": "level"}),
]
