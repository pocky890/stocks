...
# 不是理所當然繼續降。
MIN_AVG_RETURN_PCT = 2.0  # 低於這個平均報酬率才排除——門檻不是台股來回交易成本(~0.6%)，
# 是「值不值得花風險做」：只是沒虧錢但賺很少，一樣不該留著繼續佔用通知額度，2026-08-07
# 使用者確認改成5%，2026-08-17再調整成4%(跟MIN_TOTAL_RETURN_PCT一起看，不是單獨這個
# 數字變寬鬆)。2026-08-16使用者轉述Gemini建議「放寬為平均報酬>2% AND 獲利因子>1.5」，
# 查證後改採用(見下面MIN_PROFIT_FACTOR同批說明)：用scripts/backtest_looser_ranking_
# thresholds.py對現行459個(策略,股票)組合實測，4%→2%會讓27個組合從排除變保留，全部
# 樣本數都>=11筆(不是MIN_TRADES_OVERRIDES那次查證發現的1~2筆巧合)，其中17個集中在
# trend_following——這支策略本來就是「靠大量中等勝率、中等單筆報酬堆出總報酬」的樣貌
# (勝率普遍26%~39%但獲利因子2~3)，4%平均報酬門檻對這種樣貌天生不利，不是這些股票
# 真的比較差。放寬後trend_following的51檔裡從10檔保留變27檔保留，效果集中在單一策略，
# 不是均勻分散在9支策略上，使用者看過這個集中效應後仍確認採用。
...
MIN_PROFIT_FACTOR = 1.5  # 2026-08-17使用者拿10年真實回測數字比較後決定：獲利因子<2就排除，
...
#
# 2026-08-16改成1.5：使用者轉述Gemini建議「放寬為平均報酬>2% AND 獲利因子>1.5」，理由
# 是這個區間內的策略上線實戰時績效最不容易變形——這個說法本身沒有針對這份資料驗證過，
# 不能只憑聽起來合理就採用，實測(scripts/backtest_looser_ranking_thresholds.py，
# 細節見MIN_AVG_RETURN_PCT同批說明)後確認：新增的27個組合樣本數都足夠(>=11筆)，
# 不是雜訊，故採用。
