"""櫃買中心（TPEx，上櫃）免費公開資料。跟twse_client.py對應同樣的資料，但這幾個endpoint
不支援指定日期查詢，每次呼叫只回傳「最新一個交易日」的資料 -- 沒辦法backfill歷史，
只能從開始抓的那天起慢慢累積。回傳的dict裡的"date"是從回應本身的Date欄位轉換來的，
不是呼叫時的系統時間，避免週末/收盤前呼叫時日期誤判。除權息預告表2026-08-18改用FinMind
(見finmind_client.fetch_ex_dividend_schedule_for_range)，不再用這裡的官方API——那個
endpoint一樣會偶爾撞到跟三大法人/估值同一個已知的SSL憑證問題。"""
import requests
