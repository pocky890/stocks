...
永豐證券帳戶尚未開通、Shioaji API金鑰尚未申請。因此目前只完成不依賴真實Shioaji連線的部分（策略邏輯、回測框架、Telegram通知、Streamlit儀表板），全部用範例歷史資料驗證。等開戶+金鑰到手後才會接上`shioaji_client.py`的真實連線。
...
3. `cp .env.example .env`，填入 `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID`（Shioaji金鑰之後補）
...
即時監控（`run_live.py`）與全市場批次掃描（`run_batch.py`）需要真實Shioaji連線，開戶+金鑰到手後才能使用。
